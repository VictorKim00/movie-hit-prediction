...
[5] Best-5 search            best_combinations.py
    - encoder x scaler x model x hyperparameter scan -> Top-5
```
...
A reusable utility that scans every combination of **encoder × scaler × model × hyperparameter grid**, evaluates each with K-fold CV, and returns the top-k.

### Why

Manually trying every `(encoder, scaler, model, params)` combination is tedious and error-prone. This function automates the entire grid scan with a single call and reports mean score, standard deviation, and fit time per combination.
...
| Requirement | Where |
|-------------|-------|
| Data scaling & encoding | `1_preprocessing.ipynb` + `best_combinations.py` (Standard/MinMax/Robust × onehot/label/ordinal) |
| Classification algorithm | `2_modeling.ipynb` (6 models) |
| Regression or clustering | `2_modeling.ipynb` (regression — 5 models) |
